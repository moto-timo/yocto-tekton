...
NOTE: the serviceaccount.yaml is identical for meta-python, so it is normal
for it to say "unchanged".

## Limitations

- No QEMU in containers for meta-python2-ptest-image (yet), and therefore
  the testimage-task.yaml steps have not been added to the meta-python2
  pipeline
- The line getting the recipe list in build-task.yaml breaks on some
  punctuation in subject lines, e.g. "python-<recipeA>/python-<recipeB>"
